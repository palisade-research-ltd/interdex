...
pub fn q_insert_features(features: &[FeatureNew]) -> Result<String, Box<dyn error::Error>> {
    let timestamp_dt: DateTime<Utc> =
        Utc.timestamp_millis_opt(features.feature_ts as i64).unwrap();
    let timestamp = format_datetime_for_clickhouse(&timestamp_dt);

    let query = format!(
        r#"INSERT INTO 
                trades
                    (feature_ts, symbol, exchange, spread, midprice,
                     w_midprice, vwap, imb, tav)
                VALUES 
                    ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')
            "#,
        timestamp,
