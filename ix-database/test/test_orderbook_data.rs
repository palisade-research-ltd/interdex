...
mod tests {
    ...
    use chrono::Utc;
    use ix_cex::models::orderbook::{OrderBook as Orderbook, OrderbookInput, PriceLevel, PriceLevelInput};
    ...
    fn test_price_level_creation() {
        ...
        assert_eq!(level.price_f64().unwrap(), 151.52);
        assert_eq!(level.quantity_f64().unwrap(), 95.568);
    }
    ...
    fn test_orderbook_calculations() {
        let bids = vec![
            ...
            PriceLevel::new("99.0", "5.0"),
        ];
        let asks = vec![
            PriceLevel::new("101.0", "8.0"),
            PriceLevel::new("102.0", "12.0"),
        ];

        let orderbook =
            Orderbook::new("TEST", "Exchange", Utc::now(), bids, asks, 12345, None);
